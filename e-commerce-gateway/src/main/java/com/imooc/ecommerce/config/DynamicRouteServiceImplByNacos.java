 ...
 *
 * @author zzy
 * @DependsOn({"GatewayConfig"}) 在一个bean 初始化之后再去初始化 ；bean中默认第一个字母是 小写
 * @date 2022/4/16
 */
@Slf4j
@Component
...
public class DynamicRouteServiceImplByNacos {
     ...
     *
     * @PostConstruct 保证 bean 在容器中构造完成后会立即去执行此方法
     */
    @PostConstruct
    public void init() {
        ...
        try {
            ...
            if (!CollectionUtils.isEmpty(definitionList)){
                definitionList.forEach(dl ->{
                    log.info("init gateway config: [{}]",dl.toString());
                    dynamicRouteService.addRouteDefinition(dl);
                });
            }
    ...
    private void dynamicRouteByNacosListener(String dataId, String group) {
        ...
        try {
            ...
            log.error("dynamic update gateway config error: [{}]", ex.getErrMsg(), ex);
        }
    }
}
